...
class MissionButtons(discord.ui.View):
  """
  Mission buttons (Complete/Remove) class.
  """
...
class SelectComplete(discord.ui.Select):
  ...
  def __init__(self, all_missions=None):
    options = []
    self.all_missions = all_missions
    # all_missions: [[mission_name, points], ...]
  ...
  async def callback(self, interaction: discord.Interaction):
    async with bot.db.cursor() as cur:
      ...
      # Obtaining the user's points and the mission's points.
...
class SelectEdit(discord.ui.Select):
  ...
  async def callback(self, interaction: discord.Interaction):
    # self.values[0] = 'name of the mission you chose'
    mission_name = self.values[0]
    info = self.info
    ...
    async with bot.db.cursor() as cur:
      ...
      # results: [(mission_id, 'desc', points)]
...
class SelectEditOffense(discord.ui.Select):
  """
  Shows all your offenses to allow picking the one you want to edit.
  """
...
async def angel(interaction: discord.Interaction):
  """
  A debug command that removes the host from the database. 
  Only the host can use it (or whoever has their ID in MY_ID).
  """
...
async def offense(interaction: discord.Interaction):
  """
  Sends a form to add an offense.
  """
...
async def help(interaction: discord.Interaction):
  """
  Shows all commands and some info on the bot.
  """
...
async def empty_error(interaction, thing):
  """
  Sends a message that something's empty.
  """
...
async def init_user(name, id):
  """
  Initializes the user in the database if they aren't in.
  """
  async with bot.db.cursor() as cur:
